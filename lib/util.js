...
/**
 * TableStore SDK 使用的工具方法集合。
 *
 * @!attribute abort
 *   从迭代器函数 {each} 或 {arrayEach} 中返回此值
 *   以跳出迭代。
 *   @example 跳出迭代器函数
...
TableStore.util = {
  ...
  date: {
    ...
    /**
     * @return [Date] 当前的 JavaScript 日期对象。由于所有
     *   TableStore 服务都依赖于此日期对象，您可以重写
     *   此函数以为 TableStore 服务请求提供特殊的时间值。
     */
    ...
    from: function format(date) {
      if (typeof date === 'number') {
        if (date.toString().length == 10) {
          return new Date(date * 1000); // unix 时间戳（秒）
        }
        return new Date(date); // unix 时间戳（毫秒）
      }
    ...
    /**
     * 给定一个 Date 或类似日期的值，此函数将日期格式化为
     * 请求值的字符串。
     * @param [String,number,Date] date
     * @param [String] formatter 有效格式包括：
     #   * 'iso8601'
     #   * 'rfc822'
     #   * 'unixSeconds'
     #   * 'unixMilliseconds'
