...
TableStore.NodeHttpClient = TableStore.util.inherit({
  handleRequest: function handleRequest(httpRequest, httpOptions, callback, errCallback) {
    ...
    delete options.proxy; // proxy 不是 HTTP 选项
    delete options.timeout; // timeout 不是 HTTP 选项
    delete options.maxSockets; // maxSockets 不是 HTTP 选项

    var stream = http.request(options, function (httpResp) {
      callback(httpResp);
      httpResp.emit('headers', httpResp.statusCode, httpResp.headers);
    });
    httpRequest.stream = stream; // 将流附加到 httpRequest
    ...
    stream.once('timeout', function () {
      ...
      // HACK - 中止连接而不触发我们的错误处理程序
      // 因为我们已经抛出了 TimeoutError。否则连接
      // 会返回 ECONNRESET，这不是一个有用的错误消息
  ...
  writeBody: function writeBody(stream, httpRequest) {
    ...
    if (body && WritableStream && ReadableStream) { // 进度支持
