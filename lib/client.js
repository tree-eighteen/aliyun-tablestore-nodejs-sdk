...
TableStore.Client = inherit({
  ...
  /**
   * 为 getBucketLocation 提供专门的解析器 -- 所有其他
   * 操作都由超类解析。
   *
   * @api private
   */
   ...
   * 使用给定的输入参数在服务上调用操作。
   *
   * @param operation [String] 要在服务上调用的操作名称。
   * @param params [map] 操作的输入选项映射
   * @callback callback function(err, data)
   *   如果提供了回调函数，当从服务返回响应时会调用它。
   *   @param err [Error] 从请求返回的错误对象。
   *     如果请求成功则设置为 `null`。
   *   @param data [Object] 从请求返回的反序列化数据。
   *     如果发生请求错误则设置为 `null`。
   */
  ...
  /**
   * 为数据表添加预定义列。预定义列可用作索引表的主键列或者属性列。
   */
  addDefinedColumn: function addDefinedColumn(params, callback) {
    return this.makeRequest('addDefinedColumn', params, callback);
  },

  /**
   * 删除数据表上的预定义列。
   */
  deleteDefinedColumn: function deleteDefinedColumn(params, callback) {
    return this.makeRequest('deleteDefinedColumn', params, callback);
  },
