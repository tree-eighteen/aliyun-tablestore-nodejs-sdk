...
TableStore.encoder = {
    ...
    encodeAddDefinedColumn: function (params) {
        var properties = {
            tableName: params.tableName,
            columns: []
        };
        ...
        // 处理预定义列数组
        if (params.columns && Object.prototype.toString.call(params.columns) === '[object Array]') {
            TableStore.util.arrayEach(params.columns, function(column) {
                properties.columns.push({
                    name: column.name,
                    type: TableStore.DefinedColumnType[column.type] || column.type
                });
            });
        }

        var request = tsProtos.AddDefinedColumnRequest.create(properties);

        return request;
    },

    encodeDeleteDefinedColumn: function (params) {
        var properties = {
            tableName: params.tableName,
            columns: params.columns || []
        };

        var request = tsProtos.DeleteDefinedColumnRequest.create(properties);
...
var tsMap = {
    ...
    'addDefinedColumn': TableStore.encoder.encodeAddDefinedColumn,
    'deleteDefinedColumn': TableStore.encoder.encodeDeleteDefinedColumn,
